...
const MODEL_OPTIONS = [
  ...
  { id: "dev/image-to-image", label: "Flux-Dev/Image Edit" },
  { id: "edit", label: "Nano Banana 2/Edit" },
];
...
const getModelLabel = (modelId) => {
  if (modelId === "dev/image-to-image") return "Flux-Dev";
  if (modelId === "edit") return "Banana Edit";
  return modelId || "Unknown";
};

export default function ImageToImageHistory({ onClose, onSelect }) {
  ...
  const dbHistoryItems = historyByType?.imageToImage || [];
  ...
  const activeModelLabel = MODEL_OPTIONS.find((m) => m.id === selectedModel)?.label || "All Models";
      ...
      <div className="relative flex w-full max-w-5xl h-[700px] flex-col rounded-[32px] border border-slate-100 bg-white p-6 md:p-8 shadow-[0_32px_80px_rgba(15,23,42,0.18)] animate-[slideIn_0.32s_cubic-bezier(0.16,1,0.3,1)] overflow-hidden">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 shadow-[0_4px_14px_rgba(37,99,235,0.12)] border border-blue-100/50">
        ...
        <div className="flex flex-wrap items-center gap-3.5 mt-5 mb-5 shrink-0">
          ...
          <div className="flex-1 min-w-[260px] relative">
            ...
            <input
              ...
              className="w-full h-11 pl-11 pr-4 rounded-2xl border border-slate-200 bg-slate-50/30 text-sm text-slate-750 placeholder:text-slate-400 outline-none focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-500/5 transition-all shadow-sm"
            />
          </div>

          {/* Custom Model Dropdown */}
          <div ref={modelRef} className="relative z-40">
            <button
              onClick={() => {
                setModelDropdownOpen(!modelDropdownOpen);
                setSortDropdownOpen(false);
              }}
              className="h-11 rounded-2xl border border-slate-200 bg-white pl-4 pr-10 text-sm font-bold text-slate-700 hover:border-blue-350 hover:bg-slate-50/40 transition-all flex items-center gap-2 cursor-pointer shadow-sm min-w-[140px]"
            >
            ...
            {modelDropdownOpen && (
              <div className="absolute left-0 mt-2 z-50 w-48 rounded-2xl border border-slate-100 bg-white/95 p-1.5 shadow-[0_12px_36px_rgba(15,23,42,0.12)] backdrop-blur-md animate-[slideDown_0.2s_ease-out]">
                {MODEL_OPTIONS.map((opt) => (
                  ...
                  >
                    <span>{opt.label}</span>
                    {selectedModel === opt.id && <RiCheckLine size={14} className="text-blue-600" />}
                  </button>
                ))}
              </div>
            )}
          ...
          <div ref={sortRef} className="relative z-40">
            <button
              ...
              className="h-11 rounded-2xl border border-slate-200 bg-white pl-4 pr-10 text-sm font-bold text-slate-700 hover:border-blue-355 hover:bg-slate-50/40 transition-all flex items-center gap-2 cursor-pointer shadow-sm min-w-[140px]"
            >
            ...
            {sortDropdownOpen && (
              <div className="absolute right-0 mt-2 z-50 w-44 rounded-2xl border border-slate-100 bg-white/95 p-1.5 shadow-[0_12px_36px_rgba(15,23,42,0.12)] backdrop-blur-md animate-[slideDown_0.2s_ease-out]">
                {SORT_OPTIONS.map((opt) => (
                  ...
                  >
                    <span>{opt.label}</span>
                    {sortOrder === opt.id && <RiCheckLine size={14} className="text-blue-600" />}
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="flex items-center gap-1 bg-slate-100 rounded-2xl p-1 border border-slate-200/40 shadow-inner shrink-0">
            <button
              ...
              className={`h-9 w-9 rounded-xl flex items-center justify-center transition-all ${
                viewType === "grid"
                  ? "bg-white text-blue-650 shadow-sm border border-slate-200/10 font-bold"
          ...
          {paginatedItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-4 py-20 border border-dashed border-slate-200 rounded-[24px] bg-slate-50/20">
              ...
              <div className="text-center">
                ...
                <p className="mt-1.5 text-xs text-slate-400">Try adjusting your search query or model options.</p>
              </div>
            </div>
          ) : viewType === "grid" ? (
              ...
              {paginatedItems.map((item) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute right-3 bottom-3 flex h-8 w-8 items-center justify-center rounded-xl bg-white opacity-0 shadow-md translate-y-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0 hover:bg-blue-500 hover:text-white cursor-pointer"
                  >
              ...
              {paginatedItems.map((item) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-250 bg-white hover:border-blue-500 hover:bg-blue-500 hover:text-white transition-all shadow-sm hover:scale-105 active:scale-95 cursor-pointer"
                  >
            ...
            {Array.from({ length: totalPages }, (_, idx) => idx + 1).map((page) => (
              <button
                ...
                className={`h-9 w-9 rounded-xl flex items-center justify-center text-sm font-bold transition-all shadow-sm cursor-pointer ${
                  activePage === page
                    ? "border-2 border-blue-500 bg-blue-50 text-blue-600 shadow-[0_4px_12px_rgba(37,99,235,0.12)]"
