...
import { generateImageToImageModel } from "../services/text-to-image/imageToImage.service.js";
import wrapAsync from "../utils/wrapAsync.js";
import ExpressError from "../utils/ExpressError.js";
import User from "../models/user.model.js";
import Generation from "../models/generation.model.js";
import { getImageToImageCost, getTotalGenerationCost } from "../services/text-to-image/generation.helpers.js";
...
export const generateImageToImage = wrapAsync(async (req, res) => {
    ...
    files.map((file) => {
      return new Promise((resolve, reject) =>
        cloudinary.uploader
          .upload_stream(
            {
              folder: "lumora-ai/image-to-image",
            },
            (error, result) => {
  ...
  let result;
  try {
    result = await generateImageToImageModel(promptObj);
  } catch (error) {
    // Refund credits on failure
    await User.findByIdAndUpdate(user._id, { $inc: { credits: cost } });
    throw error;
